...
func move_window_in_circle(pivot: Vector2,window: Window, time: float) -> void: #moves window in offset in a circle
	var window_size: Vector2 = window.get_size() #Get soul window size
	var offset: Vector2 = pivot - window_size/2.0 #Make an offset using those variables
